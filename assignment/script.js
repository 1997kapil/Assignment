...
function renderStrategyCards() {
    ...
    const strategiesForDate = strategyArray.find(view => view.View === currentView).Value[currentDate] || [];
    ...
    if (strategiesForDate.length === 0) {
        ...
        strategyCards.innerHTML = '';
        for (const [strategyName, count] of Object.entries(strategyCount)) {
            const card = document.createElement('div');
            card.classList.add('card');
            
            const strategyCountLabel = count > 1 ? 'Strategies' : 'Strategy';
            
            card.innerHTML = `
                <h4>${strategyName}</h4>
                <p>${count} ${strategyCountLabel}</p>
            `;
